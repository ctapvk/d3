...
function createViewers(barSize){
...
function showRects(barSize){
	g.selectAll(".bar")
		  .attr("x", function(d) {
				...
				return x1(d.code % 2) +razz ; 
				})
		  .attr("y", function(d) {
				//console.log("y=" + y(d.value));
				return y(d.value); 
				})
		  .attr("width", barSize - prop.spaceBetween/2)
...
function showGridForX(barSize){
  ...
  d3.selectAll(".tickX").data(data);*/
	...
	tickText
	  .attr("x",function(row) {return x0(row.State) + prop.spaceBetween ; })
					...
					.attr("text-anchor", "start")
						.selectAll("g")
							.data(keys.slice() )
								.enter().append("g")
									.attr("class" , "legend")
									.attr("transform", function(d, i) { return "translate(" + (i * 290) + ", 0 )"; })
	;
	legend.append("rect")
				.attr("x", 10)
